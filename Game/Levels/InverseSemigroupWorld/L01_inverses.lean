...
The first of these identities is called `mul_inv` and the second is called `inv_mul`. Together with `mul_assoc` they form the axioms of an inverse semigroup.
...
NewTheorem MyInvSemigroup.mul_inv MyInvSemigroup.inv_mul

variable {S : Type}

/-- Let $S$ be an inverse semigroup. Then for all $a ∈ S$ there exists a unique inverse $a⁻¹ ∈ S$ such that $a * a⁻¹ * a = a$ and $a⁻¹ * a * a⁻¹ = a⁻¹$ -/
Statement [inst : MyInvSemigroup S] (a : S) : a * a⁻¹ * a = a ∧ a⁻¹ * a * a⁻¹ = a⁻¹ := by
  constructor
  · rewrite [mul_inv]
    rfl
  · rewrite [inv_mul]
